...
function evaluateRule(
  ...
  switch (rule.field) {
    ...
    case "album":
      ...
      break;
    case "year":
      fieldValue = new Date(song.dateAdded).getFullYear();
      break;
    case "duration":
      fieldValue = song.duration;
      break;
    case "playCount":
      fieldValue = stats[song.id]?.playCount ?? 0;
      break;
    case "lastPlayed":
      fieldValue = stats[song.id]?.lastPlayed ?? 0;
      break;
    case "dateAdded":
      fieldValue = song.dateAdded;
      break;
    default:
      return true;
  }
  ...
  switch (rule.operator) {
    ...
    case "between":
      return (
        ...
        Number(fieldValue) <= Number(rule.value2 ?? 0)
      );
    default:
      return true;
  }
  ...
  filtered.sort((a, b) => {
    ...
    switch (playlist.sortBy) {
      ...
      case "playCount":
        ...
        break;
      case "lastPlayed":
        cmp = (stats[a.id]?.lastPlayed ?? 0) - (stats[b.id]?.lastPlayed ?? 0);
        break;
    }
...
export const useSmartPlaylistStore = create<SmartPlaylistState>()(
  immer((set, get) => ({
    ...
    resolveSongs: (id, allSongs, trackStats) => {
      ...
      if (builtIn) {
        // Deep-clone rules to apply dynamic values at query time
        const resolved = {
          ...builtIn,
          rules: builtIn.rules.map((r) => ({ ...r })),
        };
        if (id === "__recently_added") {
          const recentRule = resolved.rules.find(
            (r) => r.field === "dateAdded",
          );
          if (recentRule)
            recentRule.value = Date.now() - 7 * 24 * 60 * 60 * 1000;
        }
        return resolvePlaylist(allSongs, resolved, trackStats);
      }
